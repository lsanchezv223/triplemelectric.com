...
function getPaidAmount(entry: OverviewRecord) {
...
export function AdminOverviewRecords({ entries }: { entries: OverviewRecord[] }) {
  const [detailsViewer, setDetailsViewer] = useState<OverviewRecord | null>(null);
  ...
  async function openAttachments(entry: OverviewRecord) {
    setAttachmentsError("");
    ...
    try {
      ...
      setAttachmentsViewer({
        ...
        subtitle: `${entry.user.fullName} · ${formatFullDate(new Date(`${entry.workDate.slice(0, 10)}T00:00:00Z`))}`,
    ...
    <>
        ...
        <div className="mt-6 space-y-3">
          {entries.length ? (
            ...
            <div className="rounded-[1.25rem] border border-dashed border-white/10 bg-white/[0.02] p-5 text-sm text-sand/65">
              No recent records were found for this period.
            </div>
          )}
        </div>
      </article>
      ...
      {detailsViewer ? (
        <EntryDetailsModal
          ...
          subtitle={`${detailsViewer.user.fullName} · ${formatFullDate(new Date(`${detailsViewer.workDate.slice(0, 10)}T00:00:00Z`))}`}
          statusLabel={
